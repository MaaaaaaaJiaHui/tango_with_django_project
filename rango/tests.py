...
class Chapter6IndexViewTests(TestCase):
    ...
    def test_index_context_dictionary(self):
        ...
        expected_pages_order = list(Page.objects.order_by('-views')[
                                    :5])  # From the exercises section of Chapter 6 -- we cannot assume a set order,
        # because the developer can set the number of views to whatever they wish.
        ...
        self.assertTrue('boldmessage' in self.response.context,
                        f"{FAILURE_HEADER}The 'boldmessage' variable couldn't be found in the context dictionary for "
                        f"the index() view. Did you delete it?{FAILURE_FOOTER}")
        ...
        # Check that categories exists in the context dictionary, that it references the correct objects,
        # and the order is spot on.
        self.assertTrue('categories' in self.response.context,
                        f"{FAILURE_HEADER}We couldn't find a 'categories' variable in the context dictionary within "
                        f"the index() view. Check the instructions in the book, and try again.{FAILURE_FOOTER}")
        ...
        self.assertEqual(expected_categories_order, list(self.response.context['categories']),
                         f"{FAILURE_HEADER}Incorrect categories/category order returned from the index() view's "
                         f"context dictionary -- expected {expected_categories_order}; got "
                         f"{list(self.response.context['categories'])}.{FAILURE_FOOTER}")

        # Repeat, but for the pages variable. Note that order cannot be verfified (no instructions in book to use
        # certain values).
        self.assertTrue('pages' in self.response.context,
                        f"{FAILURE_HEADER}We couldn't find a 'pages' variable in the index() view's context "
                        f"dictionary. Did you complete the Chapter 6 exercises?{FAILURE_FOOTER}")
        self.assertEqual(type(self.response.context['pages']), QuerySet,
                         f"{FAILURE_HEADER}The 'pages' variable in the index() view's context dictionary doesn't "
                         f"return a QuerySet as expected.{FAILURE_FOOTER}")
        self.assertEqual(expected_pages_order, list(self.response.context['pages']),
                         f"{FAILURE_HEADER}The 'pages' context dictionary variable for the index() view didn't return "
                         f"the QuerySet we were expectecting: got {list(self.response.context['pages'])}, "
                         f"expected {expected_pages_order}. Did you apply the correct ordering to the filtered "
                         f"results?{FAILURE_FOOTER}")

    def test_index_categories(self):
        """
        Checks the response generated by the index() view -- does it render the categories correctly? Regular
        expressions are used here (yikes) to try and be as fair as possible when checking the markup received from
        the developer's project.
        """
        category_li_entries_regex = [  # 0 = regex match, 1 = title of category, 2 = sanitised markup for error message
            [
                r'<li>(\s*|\n*)<a(\s+)href(\s*)=(\s*)("/rango/category/python/"|\'/rango/category/python/\')(\s*)>('
                r'\s*|\n*)Python(\s*|\n*)</a>(\s*|\n*)</li>',
                'Python', '<li><a href="/rango/category/python/">Python</a></li>'],
            [
                r'<li>(\s*|\n*)<a(\s+)href(\s*)=(\s*)("/rango/category/django/"|\'/rango/category/django/\')(\s*)>('
                r'\s*|\n*)Django(\s*|\n*)</a>(\s*|\n*)</li>',
                'Django', '<li><a href="/rango/category/django/">Django</a></li>'],
            [
                r'<li>(\s*|\n*)<a(\s+)href(\s*)=(\s*)("/rango/category/other-frameworks/"|\'/rango/category/other'
                r'-frameworks/\')(\s*)>(\s*|\n*)Other Frameworks(\s*|\n*)</a>(\s*|\n*)</li>',
    ...
    def test_index_pages(self):
        ...
        page_li_entries_regex = {
            'Official Python Tutorial': r'<li>(\s*|\n*)<a(\s+)href(\s*)=(\s*)('
                                        r'"http://docs.python.org/3/tutorial/"|\'http://docs.python.org/3/tutorial'
                                        r'/\')(\s*)>(\s*|\n*)Official Python Tutorial(\s*|\n*)</a>(\s*|\n*)</li>',
            'How to Think like a Computer Scientist': r'<li>(\s*|\n*)<a(\s+)href(\s*)=(\s*)('
                                                      r'"http://www.greenteapress.com/thinkpython/"|\'http://www'
                                                      r'.greenteapress.com/thinkpython/\')(\s*)>(\s*|\n*)How to Think '
                                                      r'like a Computer Scientist(\s*|\n*)</a>(\s*|\n*)</li>',
            'Learn Python in 10 Minutes': r'<li>(\s*|\n*)<a(\s+)href(\s*)=(\s*)('
                                          r'"http://www.korokithakis.net/tutorials/python/"|\'http://www.korokithakis'
                                          r'.net/tutorials/python/\')(\s*)>(\s*|\n*)Learn Python in 10 Minutes('
                                          r'\s*|\n*)</a>(\s*|\n*)</li>',
            'Official Django Tutorial': r'<li>(\s*|\n*)<a(\s+)href(\s*)=(\s*)('
                                        r'"https://docs.djangoproject.com/en/2.1/intro/tutorial01/"|\'https://docs'
                                        r'.djangoproject.com/en/2.1/intro/tutorial01/\')(\s*)>(\s*|\n*)Official '
                                        r'Django Tutorial(\s*|\n*)</a>(\s*|\n*)</li>',
            'Django Rocks': r'<li>(\s*|\n*)<a(\s+)href(\s*)=(\s*)('
                            r'"http://www.djangorocks.com/"|\'http://www.djangorocks.com/\')(\s*)>(\s*|\n*)Django '
                            r'Rocks(\s*|\n*)</a>(\s*|\n*)</li>',
            'How to Tango with Django': r'<li>(\s*|\n*)<a(\s+)href(\s*)=(\s*)('
                                        r'"http://www.tangowithdjango.com/"|\'http://www.tangowithdjango.com/\')('
                                        r'\s*)>(\s*|\n*)How to Tango with Django(\s*|\n*)</a>(\s*|\n*)</li>',
            'Bottle': r'<li>(\s*|\n*)<a(\s+)href(\s*)=(\s*)('
                      r'"http://bottlepy.org/docs/dev/"|\'http://bottlepy.org/docs/dev/\')(\s*)>(\s*|\n*)Bottle('
                      r'\s*|\n*)</a>(\s*|\n*)</li>',
            'Flask': r'<li>(\s*|\n*)<a(\s+)href(\s*)=(\s*)("http://flask.pocoo.org"|\'http://flask.pocoo.org\')('
                     r'\s*)>(\s*|\n*)Flask(\s*|\n*)</a>(\s*|\n*)</li>',
        }
        ...
        for expected_regex in expected_pages_li:
            ...
            self.assertTrue(re.search(expected_regex, self.content),
                            f"{FAILURE_HEADER}Checks for the top five pages in the index() view's response failed. "
                            f"Check you are using the correct list of objects, the correct HTML markup, "
                            f"and try again. '{expected_regex}'{FAILURE_FOOTER}")
    ...
    def test_index_response_titles(self):
        ...
        self.assertIn(expected_category_h2, self.content,
                      f"{FAILURE_HEADER}We couldn't find the markup '{expected_category_h2}' in your index.html "
                      f"template. Check you completed the Chapter 6 exercises as requested, and try again."
                      f"{FAILURE_FOOTER}")
        self.assertIn(expected_page_h2, self.content,
                      f"{FAILURE_HEADER}We couldn't find the markup '{expected_page_h2}' in your index.html template. "
                      f"Check you completed the Chapter 6 exercises as requested, and try again.{FAILURE_FOOTER}")
...
class Chapter6NoItemsIndexViewTests(TestCase):
    ...
    def test_empty_index_context_dictionary(self):
        """
        Runs assertions on the context dictionary, ensuring the categories and pages variables exist, but return
        empty (zero-length) QuerySet objects.
        """
        self.assertTrue('categories' in self.response.context,
                        f"{FAILURE_HEADER}The 'categories' variable does not exist in the context dictionary for "
                        f"index(). (Empty check){FAILURE_FOOTER}")
        self.assertEqual(type(self.response.context['categories']), QuerySet,
                         f"{FAILURE_HEADER}The 'categories' variable in the context dictionary for index() does yield "
                         f"a QuerySet object. (Empty check){FAILURE_FOOTER}")
        self.assertEqual(len(self.response.context['categories']), 0,
                         f"{FAILURE_HEADER}The 'categories' variable in the context dictionary for index() is not "
                         f"empty. (Empty check){FAILURE_FOOTER}")

        self.assertTrue('pages' in self.response.context,
                        f"{FAILURE_HEADER}The 'pages' variable does not exist in the context dictionary for index(). "
                        f"(Empty check){FAILURE_FOOTER}")
        self.assertEqual(type(self.response.context['pages']), QuerySet,
                         f"{FAILURE_HEADER}The 'pages' variable in the context dictionary for index() does yield a "
                         f"QuerySet object. (Empty check){FAILURE_FOOTER}")
        self.assertEqual(len(self.response.context['pages']), 0,
                         f"{FAILURE_HEADER}The 'pages' variable in the context dictionary for index() is not empty. ("
                         f"Empty check){FAILURE_FOOTER}")
    ...
    def test_empty_index_response(self):
        ...
        self.assertIn('<strong>There are no categories present.</strong>', self.content,
                      f"{FAILURE_HEADER}When no categories are present, we can't find the required '<strong>There are "
                      f"no categories present.</strong>' markup in your index() view's output.{FAILURE_FOOTER}")
        self.assertIn('<strong>There are no pages present.</strong>', self.content,
                      f"{FAILURE_HEADER}When no categories are present, we can't find the required '<strong>There are "
                      f"no pages present.</strong>' markup in your index() view's output. Read the Chapter 6 "
                      f"exercises carefully.{FAILURE_FOOTER}")
    ...
    def test_sample_category(self):
        ...
        category_regex = r'<li>(\s*|\n*)<a(\s+)href(\s*)=(\s*)(' \
                         r'"/rango/category/test-category/"|\'/rango/category/test-category/\')(\s*)>(\s*|\n*)Test ' \
                         r'Category(\s*|\n*)</a>(\s*|\n*)</li> '
        self.assertTrue(re.search(category_regex, updated_response),
                        f"{FAILURE_HEADER}When adding a test category, we couldn't find the markup for it in the "
                        f"output of the index() view. Check you have included all the code correctly for displaying "
                        f"categories.{FAILURE_FOOTER}")
        self.assertIn('<strong>There are no pages present.</strong>', self.content,
                      f"{FAILURE_HEADER}When no categories are present, we can't find the required '<strong>There are "
                      f"no pages present.</strong>' markup in your index() view's output. Read the Chapter 6 "
                      f"exercises carefully.{FAILURE_FOOTER}")
...
class Chapter6CategoryViewTests(TestCase):
    ...
    def test_template_filename(self):
        ...
        self.assertTemplateUsed(self.response, 'rango/category.html',
                                f"{FAILURE_HEADER}The category.html template is not used for the show_category() "
                                f"view. The specification requires this.{FAILURE_FOOTER}")
    ...
    def test_slug_functionality(self):
        ...
        self.assertEquals('unscrupulous-nonsense', category.slug,
                          f"{FAILURE_HEADER}When changing the name of a category, the slug attribute was not updated "
                          f"(correctly) to reflect this change. Did you override the save() method in the Category "
                          f"model correctly?{FAILURE_FOOTER}")
    ...
    def test_context_dictionary(self):
        ...
        self.assertTrue('category' in self.response.context,
                        f"{FAILURE_HEADER}The 'category' variable in the context dictionary for the show_category() "
                        f"view was not found. Did you spell it correctly?{FAILURE_FOOTER}")
        self.assertTrue('pages' in self.response.context,
                        f"{FAILURE_HEADER}The 'pages' variable in the context dictionary for the show_category() view "
                        f"was not found.{FAILURE_FOOTER}")

        self.assertEqual(self.response.context['category'], other_frameworks_category,
                         f"{FAILURE_HEADER}The category returned in the context dictionary for the show_category() "
                         f"view did not match what was expected. We expect to see a Category object returned here ("
                         f"specifically the 'Other Frameworks' category, for our tests).{FAILURE_FOOTER}")
        self.assertEqual(list(self.response.context['pages']), page_list,
                         f"{FAILURE_HEADER}The list of pages returned in the context dictionary of the show_category("
                         f") view was not correct. Did you filter the pages correctly in your view?{FAILURE_FOOTER}")
    ...
    def test_response_markup(self):
        ...
        self.assertIn(expected_header, self.content,
                      f"{FAILURE_HEADER}The header tag '{expected_header}' was not found in the response for the "
                      f"show_category() view. Make sure the category.html template matches the specification."
                      f"{FAILURE_FOOTER}")
        self.assertTrue(re.search(bottle_markup, self.content),
                        f"{FAILURE_HEADER}Correctly formed <li> markup was not found for the pages to be displayed in "
                        f"the show_category() view. Make sure your category.html template is well-formed!"
                        f"{FAILURE_FOOTER}")
    ...
    def test_for_homepage_link(self):
        ...
        self.assertTrue(re.search(homepage_hyperlink_markup, self.content),
                        f"{FAILURE_HEADER}We couldn't find a well-formed hyperlink to the Rango homepage in your "
                        f"category.html template. This is an exercise at the end of Chapter 6.{FAILURE_FOOTER}")
...
class Chapter6BadCategoryViewTests(TestCase):
    ...
    def test_malformed_url(self):
        ...
        self.assertTrue(response.status_code == 404,
                        f"{FAILURE_HEADER}The URL /rango/category/ should return a status of code of 404 (not found). "
                        f"Check to see whether you have correctly entered your urlpatterns.{FAILURE_FOOTER}")
    ...
    def test_nonexistent_category(self):
        ...
        self.assertIn(lookup_string, response.content.decode(),
                      r"{FAILURE_HEADER}The expected message when attempting to access a non-existent category was "
                      r"not found. Check your category.html template.{FAILURE_FOOTER}")
